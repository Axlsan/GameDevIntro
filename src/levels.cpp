...
void CreateLevel(Arena *arena, LevelData *level, const char *levelName) {
  ...
  vector dataField =
      jsonResult["layers"][LEVEL_INDEX]["data"].get<vector<uint8_t>>();
...
void CreateEntities(LevelData *lvlData, Arena *arena) {
  ...
  auto entityData =
      result["layers"][ENTITIES_INDEX]["data"].get<vector<uint8_t>>();

  lvlData->entityBuffer = ALLOC_ARRAY(arena, Entity, lvlData->w * lvlData->h);
  // lvlDarta->entityBuffer = (Entity*)Memory::Allocate(arena, sizeof(Entity) *
  // lvlData->w * lvlData->h); lvlData->entityBuffer =
  // (Entity*)Memory::Allocate(arena, sizeof(Entity) * lvlData->entityCount);
...
Entity *RaycastFirstEntity(int originX, int originY, Direction direction,
                           LevelData *lvl, bool ignoreWalls) {

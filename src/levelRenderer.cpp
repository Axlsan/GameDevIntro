  ...
  for (int x = 0; x < lvlData.w; x++) {
    for (int y = 0; y < lvlData.h; y++) {
      ...
      if (ID(cellType) == ID::GROUND) {
        sprite =
            &gameData
                 ->spriteBuffer[(x + y) % 2 == 0 ? (int)SPRITE_ID::Ground
                                                 : (int)SPRITE_ID::GroundAlt];
      } else {
...
void RenderEntities(GameData *data, SDL_Renderer *renderer) {
  ...
  std::sort(SortedEntities, SortedEntities + lvlData->entityCount,
  ...
  for (int i = 0; i < lvlData->entityCount; i++) {
    ...
    Sprite *sprite = GetSpriteFromEntityState(entity, data->spriteBuffer);
    ...
    if (HasBehaviour(entity, Behaviour::JUMPS) &&
    ...
    RenderEntity_OnTile(dropshadow, lvlData, renderer, &data->camera, xAnimated,
                        dropshadowY, 1, 0.4, false);
    RenderEntity_OnTile(sprite, lvlData, renderer, &data->camera, xAnimated,
                        yAnimated, 1, 1, entity->facing == Direction::RIGHT);

    // RenderSprite_Grid(sprite, &lvlData, renderer, &data->camera, xAnimated,
    // yAnimated);
  }
}
